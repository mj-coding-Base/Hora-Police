#!/usr/bin/env bash
# Safe deployment script for Hora-Police binary
# Copies binary from /tmp/hora-police to /usr/local/bin/hora-police
# Ensures directories exist, reloads systemd, starts service
set -euo pipefail
...
if sudo systemctl is-active --quiet hora-police; then
    ...
    echo "  3. Check service file: sudo systemctl cat hora-police"
    exit 1
fi
